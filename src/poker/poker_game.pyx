...
cdef class PokerGame:
    ...
    cpdef get_action(self):
            if self.game_state.get_current_player().is_human:
                ...
                _, external_manager = self.strategy_trainer.train([fast_forward_actions], [player_string_hand])

                strategy = self.strategy_trainer.get_average_strategy(self.game_state.get_current_player(), self.game_state, external_manager)
    ...
    cpdef skip_to_showdown(self):
        ...
        self.game_state.debug_output()
        print(f"Player {self.game_state.winner_index} wins the hand.")
        for i in range(5):
            print(f"Starting next hand in {5-i} seconds...")
            time.sleep(1)