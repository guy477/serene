...
cdef class AIPlayer(Player):
    ...
    cpdef bint get_action(self, GameState game_state, int player_index, CFRTrainer strategy_trainer = None):
        cdef object user_input
        cdef bint valid = 0
        cdef bint raize = 0

        if not strategy_trainer:
            return random.choice(self.get_available_actions(game_state))
    ...
    cpdef public AIPlayer clone(self):
        ...
        cdef AIPlayer new_player = AIPlayer(self.chips, self.bet_sizing)
